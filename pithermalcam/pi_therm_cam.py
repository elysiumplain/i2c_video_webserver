...
# Set up logging
logging.basicConfig(filename='thermal_cam.log', filemode='a',
                    format='%(asctime)s %(levelname)-8s [%(filename)s:%(name)s:%(lineno)d] %(message)s',
                    level=logging.WARNING, datefmt='%d-%b-%y %H:%M:%S')
logger = logging.getLogger(__name__)


def _temps_to_rescaled_uints(f: List[float] | np.ndarray, Tmin: float, Tmax: float) -> np.ndarray:
...
class pithermalcam:
    ...
    _colormap_list: List[str] = ['jet', 'bwr', 'seismic', 'coolwarm', 'PiYG_r', 'tab10', 'tab20', 'gnuplot2', 'brg']
    _interpolation_list: List[int] = [cv2.INTER_NEAREST, cv2.INTER_LINEAR, cv2.INTER_AREA, cv2.INTER_CUBIC,
                                      cv2.INTER_LANCZOS4, 5, 6]
    _interpolation_list_name: List[str] = ['Nearest', 'Inter Linear', 'Inter Area', 'Inter Cubic', 'Inter Lanczos4',
                                           'Pure Scipy', 'Scipy/CV2 Mixed']
    _current_frame_processed: bool = False  # Tracks if the current processed image matches the current raw image
    ...
    def _pull_raw_image(self):
        ...
        try:
            ...
            self._raw_image = _temps_to_rescaled_uints(self._raw_image, self._temp_min, self._temp_max)
    ...
    def _add_image_text(self):
        ...
        if self._file_saved_notification_start is not None and (
                time.monotonic() - self._file_saved_notification_start) < 1:
    ...
    def _set_image_text_background(self):
        ...
        banner_height = 20
        cv2.rectangle(self._image, (0, 0), (self._image.shape[1], banner_height), (20, 20, 20),
    ...
    def get_current_image_frame(self) -> ndarray | None:
